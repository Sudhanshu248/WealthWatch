import { FoodExpence, TransportExpence, HousingExpence, SavingExpence, PersonalExpence } from "../../data/CalForthMonthExpence";
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";


export default function ForthIndividualData() {
    ...
    const value = location.pathname.replace("/cashflow/SixMonth/4/", "");
            ...
            <div className='flex flex-row '>
                ...
                <div className="bg-[#B8D7DE8C] rounded-md mt-4 ml-64  h-[100vh] w-[60vw]  grow px-12 py-8">
                    ...
                    <div className="bg-white w-full h-fit rounded-2xl mt-2 px-12 py-8">
                        ...
                        {
                            location.pathname == "/cashflow/SixMonth/4/Food" && (Foodlist || []).map((item, index) => (
                        ...
                        {
                            location.pathname == "/cashflow/SixMonth/4/Transport" && (TransportListing || []).map((item, index) => (
                                <div key={index} className="flex flex-row justify-between mt-4 border-b pt-4 pb-2 pl-2 ">
                                    <div className="flex flex-row gap-2">
                        ...
                        {
                            location.pathname == "/cashflow/SixMonth/4/Housing" && (HousingListing || []).map((item, index) => (
                                <div key={index} className="flex flex-row justify-between mt-4 border-b pt-4 pb-2 pl-2 ">
                                    <div className="flex flex-row gap-2">
                        ...
                        {
                            location.pathname == "/cashflow/SixMonth/4/Saving" && (SavingListing || []).map((item, index) => (
                                <div key={index} className="flex flex-row justify-between mt-4 border-b pt-4 pb-2 pl-2 ">
                                    <div className="flex flex-row gap-2">
                        ...
                        {
                            location.pathname == "/cashflow/SixMonth/4/PersonalExpence" && (PersonalListing || []).map((item, index) => (
                                <div key={index} className="flex flex-row justify-between mt-4 border-b pt-4 pb-2 pl-2 ">
                                    <div className="flex flex-row gap-2">

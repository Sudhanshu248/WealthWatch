import {lastMonthExpence} from "../data/lastMonthExp.js"

export const FoodExpence = (CurrExp) => {

    const TotalBudget = Number(lastMonthExpence.totalBudget?.value) || 0;

    // Formula for calculating food expense
    const FoodExpenceObj = lastMonthExpence.expence?.[0] || {};
...
export const TransportExpence = (CurrExp) => {
    const TotalBudget = Number(lastMonthExpence.totalBudget?.value) || 0;
    const TransportExpenceObj = lastMonthExpence.expence?.[1] || {};
...
export const PersonalExpence = (CurrExp) => {
    ...
    const TotalBudget = Number(lastMonthExpence.totalBudget?.value) || 0;
    const PersonalExpenceObj = lastMonthExpence.expence?.[2] || {};
...
export const HousingExpence = (CurrExp) => {
    const TotalBudget = Number(lastMonthExpence.totalBudget?.value) || 0;
    const HousingExpenceObj = lastMonthExpence.expence?.[3] || {};
...
export const SavingExpence = (CurrExp) => {
    ...
    const TotalBudget = Number(lastMonthExpence.totalBudget?.value) || 0;
    const SavingExpenceObj = lastMonthExpence.expence?.[4] || {};
...
export const TotalExpence = (CurrExp) => {
    const TotalBudget = Number(lastMonthExpence.totalBudget?.value) || 0;
    const Expence = lastMonthExpence.expence;
